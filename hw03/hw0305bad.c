...
// void monarch(uint8_t* savedata, int32_t nowIdx);
// void city(uint8_t* savedata, int32_t nowIdx);
...
void general(uint8_t* savedata, int32_t nowIdx, uint64_t filesize, int32_t year){
    ...
    while(1){
        ...
        for(int32_t i = nowIdx;i < filesize - 16;i++){
            ...
            if(memcmp(&savedata[i],end,18) == 0 || i == filesize - 1) break;
        }
...
int main(){
    ...
    while(1){
        ...
        for(int32_t i = 0;i < filesize;i++){
            ...
            if(i == filesize - 1) printf(RED"Error: No such savedata\n"RESET);
        }
        if(isFound){
            _NEW_PAGE_;
            printf(GREEN_BOLD"Success!\n"RESET);
    ...
    while(1){
        ...
        switch(choice){
            case 1:
                _NEW_PAGE_;
                //monarch(savedata, nowIdx);
                break;
            case 2:
                _NEW_PAGE_;
                //city(savedata, nowIdx);
                break;
            case 3:
                _NEW_PAGE_;
